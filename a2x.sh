...
function parse_options()
{
    ...
    long_opts="attribute:,asciidoc-opts:,destination-dir:,doctype:,help,icons-dir:,dry-run,format:,copy,icons,skip-asciidoc,stylesheet:,safe,unsafe,version,verbose,xsltproc-opts:,dblatex-opts:,fop,fop-opts:,no-xmllint"
    ...
    while true ; do
        case "$1" in
            ...
            --stylesheet)
                ...
                shift 2 ;;
            --safe)
                ASCIIDOC_OPTS="$ASCIIDOC_OPTS --safe";
                shift ;;
            # DEPRECATED: --unsafe option.
            --unsafe)
